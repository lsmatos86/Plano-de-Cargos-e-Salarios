<?php
// Arquivo: src/Repository/RiscoRepository.php
...
/**
 * Lida com todas as operações de banco de dados para a entidade Risco.
 */
class RiscoRepository
{
    private PDO $pdo;
    private string $tableName = 'riscos';
    private string $idColumn = 'riscoId';
    private string $nameColumn = 'riscoNome'; // Coluna ENUM
    ...
    /**
     * Retorna as opções válidas para o campo ENUM 'riscoNome'.
     * (Migrado de getEnumOptions() em functions.php)
     *
    ...
    {
        try {
            $stmt = $this->pdo->query("SHOW COLUMNS FROM {$this->tableName} LIKE '{$this->nameColumn}'");
            $row = $stmt->fetch();
    ...
    /**
     * Salva (cria ou atualiza) um registro de risco.
     * (Migrado de views/riscos.php)
     *
     * @param array $data Dados vindos do formulário ($_POST)
     * @return int O número de linhas afetadas.
     * @throws Exception Se o nome não for um ENUM válido.
     */
    public function save(array $data): int
    {
        $riscoNome = trim($data[$this->nameColumn] ?? '');
        $id = (int)($data[$this->idColumn] ?? 0);
        $action = $data['action'] ?? '';

        // Validação: Garante que o valor enviado seja um valor ENUM válido
        $enum_options = $this->getEnumOptions();
        if (empty($riscoNome) || !in_array($riscoNome, $enum_options)) {
            throw new Exception("O tipo de risco selecionado não é válido.");
        }
        ...
        try {
            ...
            error_log("Erro ao salvar risco: " . $e->getMessage());
            // Verifica erro de entrada duplicada
            if ($e->getCode() == 23000) { 
                throw new Exception("Erro: O risco '{$riscoNome}' já está cadastrado.");
            }
    ...
    public function delete(int $id): int
    {
        try {
            ...
            error_log("Erro ao excluir risco: " . $e->getMessage());
            if ($e->getCode() == 23000) {
                // Erro de chave estrangeira (FK)
                throw new Exception("Erro: Este risco não pode ser excluído pois está sendo utilizado por um ou mais cargos.");
            }
