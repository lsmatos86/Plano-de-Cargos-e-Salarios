...
/**
 * Lida com todas as operações de banco de dados para a entidade Caracteristica.
 */
class CaracteristicaRepository
{
    private PDO $pdo;
    private string $tableName = 'caracteristicas';
    private string $idColumn = 'caracteristicaId';
    private string $nameColumn = 'caracteristicaNome';
    private string $descColumn = 'caracteristicaDescricao';
    ...
    /**
     * Salva (cria ou atualiza) um registro de característica.
     * (Migrado de views/caracteristicas.php)
     *
    ...
    public function save(array $data): int
    {
        ...
        $descricao = trim($data[$this->descColumn] ?? null); // Aceita nulo
        $id = (int)($data[$this->idColumn] ?? 0);
        $action = $data['action'] ?? '';

        if (empty($nome)) {
            throw new Exception("O nome da característica não pode estar vazio.");
        }
        ...
        try {
            if ($action === 'insert') {
                ...
                $sql = "UPDATE {$this->tableName} SET {$this->nameColumn} = ?, {$this->descColumn} = ? WHERE {$this->idColumn} = ?";
                $stmt = $this->pdo->prepare($sql);
            ...
            error_log("Erro ao salvar característica: " . $e->getMessage());
            throw new Exception("Erro de banco de dados ao salvar. " . $e->getMessage());
        }
    }

    /**
     * Exclui um registro de característica.
     * (Migrado de views/caracteristicas.php)
     *
    ...
    public function delete(int $id): int
    {
        try {
            ...
            error_log("Erro ao excluir característica: " . $e->getMessage());
            if ($e->getCode() == 23000) {
                // Erro de chave estrangeira (FK)
                throw new Exception("Erro: Esta característica não pode ser excluída pois está sendo utilizada por um ou mais cargos.");
            }
    ...
    public function findAllPaginated(array $params = []): array
    {
        ...
        $validColumns = [$this->idColumn, $this->nameColumn, $this->descColumn, $this->idColumn.'DataCadastro', $this->idColumn.'DataAtualizacao'];
