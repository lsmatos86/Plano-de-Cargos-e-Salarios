...
class PesquisaRepository
{
    ...
    public function createCampanha(string $titulo, string $data_abertura): int
    {
        $stmt = $this->pdo->prepare(
            'INSERT INTO campanhas_pesquisa (titulo, data_abertura, status)
             VALUES (?, ?, \'Aberta\') RETURNING "campanhaId"'
        );
    ...
    public function encerrarCampanha(int $campanhaId): bool
    {
        ...
        $stmt = $this->pdo->prepare(
            'UPDATE campanhas_pesquisa
             SET status = \'Encerrada\', data_fechamento = CURRENT_DATE
             WHERE "campanhaId" = ?'
        );
    ...
    {
        $stmt = $this->pdo->prepare(
            'INSERT INTO empresas_mercado (nome, setor, porte)
             VALUES (?, ?, ?) RETURNING "empresaId"'
        );
...
// --- NOVAS FUNÇÕES PARA OS LANÇAMENTOS E ESTATÍSTICAS ---
    ...
    {
        $stmt = $this->pdo->prepare(
            'INSERT INTO pesquisa_valores
                ("campanhaId", "empresaId", "cboId", cargo_nome_mercado,
                 salario_base, ano_referencia, salario_original, foi_reajustado)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?) RETURNING "valorId"'
        );
