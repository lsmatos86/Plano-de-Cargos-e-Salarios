...
// ----------------------------------------------------
// LÓGICA DO PDF (EXECUÇÃO PESADA)
// ----------------------------------------------------
if ($start_generation && $output_format === 'pdf') {
    
    // 2. CARREGAMENTO DE TODOS OS IDS DE CARGOS
    try {
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="header-section">
                ...
                <h3>Total de Cargos: <?php echo count($cargos_base); ?></h3>
            </div>
        
        <?php foreach ($cargos_base as $index => $c_base): 
            
            // Quebra de página antes de cada cargo, exceto o primeiro
            if ($index > 0): ?>
...
// ----------------------------------------------------
// LÓGICA DO HTML (MODAL DE CARREGAMENTO)
// ----------------------------------------------------
else {
    // ... (Modal de Carregamento para o usuário - Mantido) ...
}