<?php
// Arquivo: views/caracteristicas.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if (!empty($nome)) {
        try {
            if ($action === 'insert') {
                // Lógica de Inserção com dois campos
                $stmt = $pdo->prepare("INSERT INTO {$table_name} ({$name_column}, {$desc_column}) VALUES (?, ?)");
                $stmt->execute([$nome, $descricao]);
                
                $message = "Característica '{$nome}' cadastrada com sucesso!";
                $message_type = 'success';
                
            } elseif ($action === 'update' && $id > 0) {
                // Lógica de Atualização (UPDATE)
                $stmt = $pdo->prepare("UPDATE {$table_name} SET {$name_column} = ?, {$desc_column} = ? WHERE {$id_column} = ?");
                $stmt->execute([$nome, $descricao, $id]);

                $message = "Característica ID {$id} atualizada com sucesso!";
                $message_type = 'success';
            }
        } catch (Exception $e) {
        ...
        $message = "O nome da Característica não pode ser vazio.";
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    ...
    if (deleteRecord($pdo, $table_name, $id_column, $id)) {
        ...
        $message = "Erro ao excluir: A Característica ID {$id} está vinculada a um Cargo.";
        $message_type = 'danger';
    }
    ...
    <div class="row mb-3">
        <div class="col-md-4">
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#cadastroModal" id="btnNovoCadastro">
                <i class="fas fa-plus"></i> Inserir Nova Característica
            </button>
        </div>
        <div class="col-md-8">
            <form method="GET" class="d-flex">
                <input type="search" name="term" class="form-control me-2" placeholder="Filtrar por Nome da Característica" value="<?php echo htmlspecialchars($params['term']); ?>">
                <button class="btn btn-outline-secondary" type="submit">Buscar</button>
                <?php if (!empty($params['term'])): ?>
                    <a href="caracteristicas.php" class="btn btn-outline-danger ms-2" title="Limpar Filtro"><i class="fas fa-times"></i></a>
                <?php endif; ?>
            </form>
        </div>
    </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="caracteristicas.php?action=delete&id=<?php echo $row[$id_column]; ?>" 
                                       ...
                                       title="Excluir"
                                       onclick="return confirm('ATENÇÃO: Esta característica pode estar vinculada a um Cargo. Deseja realmente excluir?');">
        ...
        <div class="modal-content">
            <form method="POST" id="formCadastro">
                ...
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="cadastroModalLabel">Cadastrar Nova Característica</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="inputNome" class="form-label">Nome da Característica</label>
                        <input type="text" class="form-control" id="inputNome" name="<?php echo $name_column; ?>" placeholder="Ex: Ética, Honestidade, Ambição Saudável" required>
                    </div>
                    <div class="mb-3">
                        <label for="inputDescricao" class="form-label">Descrição (Opcional)</label>
                        <textarea class="form-control" id="inputDescricao" name="<?php echo $desc_column; ?>" rows="3" placeholder="Detalhe o significado e importância desta característica."></textarea>
                    </div>
                </div>
