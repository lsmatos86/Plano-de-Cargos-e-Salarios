...
// 1. Obtém as opções ENUM ('Hardskill', 'Softskill') para usar no formulário SELECT
$enum_options = getEnumOptions($pdo, $table_name, $type_column);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    $tipo = trim($_POST[$type_column] ?? ''); // Valor ENUM selecionado
    $descricao = trim($_POST[$desc_column] ?? NULL);
    $id = (int)($_POST[$id_column] ?? 0);
    $action = $_POST['action'];

    if (!empty($nome) && in_array($tipo, $enum_options)) {
        try {
            if ($action === 'insert') {
                // Lógica de Inserção com três campos
                $stmt = $pdo->prepare("INSERT INTO {$table_name} ({$name_column}, {$type_column}, {$desc_column}) VALUES (?, ?, ?)");
                $stmt->execute([$nome, $tipo, $descricao]);
                
                $message = "Habilidade '{$nome}' ({$tipo}) cadastrada com sucesso!";
                $message_type = 'success';
                
            } elseif ($action === 'update' && $id > 0) {
                // Lógica de Atualização (UPDATE)
                $stmt = $pdo->prepare("UPDATE {$table_name} SET {$name_column} = ?, {$type_column} = ?, {$desc_column} = ? WHERE {$id_column} = ?");
                $stmt->execute([$nome, $tipo, $descricao, $id]);

                $message = "Habilidade ID {$id} atualizada com sucesso!";
                $message_type = 'success';
            }
        } catch (Exception $e) {
        ...
        $message = "O nome e o tipo da Habilidade são campos obrigatórios.";
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    ...
    if (deleteRecord($pdo, $table_name, $id_column, $id)) {
        ...
        $message = "Erro ao excluir: A Habilidade ID {$id} está vinculada a um Cargo.";
        $message_type = 'danger';
    }
    ...
    <div class="row mb-3">
        <div class="col-md-4">
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#cadastroModal" id="btnNovoCadastro">
                <i class="fas fa-plus"></i> Inserir Nova Habilidade
            </button>
        </div>
        <div class="col-md-8">
            <form method="GET" class="d-flex">
                <input type="search" name="term" class="form-control me-2" placeholder="Filtrar por Nome da Habilidade" value="<?php echo htmlspecialchars($params['term']); ?>">
            ...
            <table class="table table-striped table-hover table-sm mb-0">
                <thead class="bg-light">
                    <tr>
                        ...
                        <th><a href="?order_by=<?php echo $type_column; ?>&sort_dir=<?php echo getSortDirection($params['order_by'], $type_column); ?>&term=<?php echo urlencode($params['term']); ?>" class="text-decoration-none text-dark">Tipo
                            <?php if ($params['order_by'] === $type_column): ?><i class="fas fa-sort-<?php echo strtolower($params['sort_dir']); ?>"></i><?php endif; ?>
                        </a></th>
                        <th><a href="?order_by=<?php echo $name_column; ?>&sort_dir=<?php echo getSortDirection($params['order_by'], $name_column); ?>&term=<?php echo urlencode($params['term']); ?>" class="text-decoration-none text-dark">Nome
                    ...
                    <?php if (count($registros) > 0): ?>
                        <?php foreach ($registros as $row): ?>
                            <tr>
                                ...
                                <td>
                                    <span class="badge bg-<?php echo ($row[$type_column] === 'Hardskill' ? 'secondary' : 'warning text-dark'); ?>">
                                        <?php echo htmlspecialchars($row[$type_column]); ?>
                                    </span>
                                </td>
                                <td><?php echo htmlspecialchars($row[$name_column]); ?></td>
                                <td><?php echo nl2br(htmlspecialchars($row[$desc_column] ?? '')); ?></td>
                                <td>
                                    <button 
                                        type="button" 
                                    ...
                                    <a href="habilidades.php?action=delete&id=<?php echo $row[$id_column]; ?>" 
                                       ...
                                       title="Excluir"
                                       onclick="return confirm('ATENÇÃO: Esta habilidade pode estar vinculada a um Cargo. Deseja realmente excluir?');">
        ...
        <div class="modal-content">
            <form method="POST" id="formCadastro">
                ...
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="cadastroModalLabel">Cadastrar Nova Habilidade</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="inputTipo" class="form-label">Tipo de Habilidade</label>
                        <select class="form-select" id="inputTipo" name="<?php echo $type_column; ?>" required>
                            <option value="" disabled selected>Selecione (Hard Skill ou Soft Skill)</option>
                    ...
                    <div class="mb-3">
                        <label for="inputNome" class="form-label">Nome da Habilidade</label>
                        <input type="text" class="form-control" id="inputNome" name="<?php echo $name_column; ?>" placeholder="Ex: Proatividade, Liderança, Supervisão de Colheita" required>
                    </div>
                    <div class="mb-3">
                        <label for="inputDescricao" class="form-label">Descrição (Opcional)</label>
                        <textarea class="form-control" id="inputDescricao" name="<?php echo $desc_column; ?>" rows="3" placeholder="Detalhe o significado e o contexto da habilidade."></textarea>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const resetModal = () => {
        ...
        modalId.value = '';
        inputNome.value = ''; 
        inputTipo.selectedIndex = 0; // Reseta o SELECT
