...
$table_name = 'recursos';
$id_column = 'recursoId';
$name_column = 'recursoNome';
$desc_column = 'recursoDescricao';
$page_title = 'Gestão de Recursos e Equipamentos';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    $descricao = trim($_POST[$desc_column] ?? NULL); // Pode ser NULL se vazio
    ...
    if (!empty($nome)) {
        try {
            if ($action === 'insert') {
                ...
                $message = "Recurso '{$nome}' cadastrado com sucesso!";
                ...
                $message = "Recurso ID {$id} atualizado com sucesso!";
                $message_type = 'success';
            }
        } catch (Exception $e) {
        ...
        $message = "O nome do Recurso não pode ser vazio.";
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    ...
    if (deleteRecord($pdo, $table_name, $id_column, $id)) {
        ...
        $message = "Erro ao excluir: O registro ID {$id} não foi encontrado ou está vinculado a um Grupo de Recurso.";
        $message_type = 'danger';
    }
    ...
    <div class="row mb-3">
        <div class="col-md-4">
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#cadastroModal" id="btnNovoCadastro">
                <i class="fas fa-plus"></i> Inserir Novo Recurso
            </button>
        </div>
        <div class="col-md-8">
                        ...
                        <?php foreach ($registros as $row): ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="recursos.php?action=delete&id=<?php echo $row[$id_column]; ?>" 
                                       class="btn btn-sm btn-danger" 
                                       title="Excluir"
                                       onclick="return confirm('ATENÇÃO: Este recurso pode estar vinculado a grupos ou cargos. Deseja realmente excluir?');">
        ...
        <div class="modal-content">
            <form method="POST" id="formCadastro">
                ...
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="cadastroModalLabel">Cadastrar Novo Recurso</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="inputNome" class="form-label">Nome do Recurso</label>
                        <input type="text" class="form-control" id="inputNome" name="<?php echo $name_column; ?>" placeholder="Ex: Rádios, Celulares" required>
                    </div>
                    <div class="mb-3">
                        <label for="inputDescricao" class="form-label">Descrição (Opcional)</label>
                        <textarea class="form-control" id="inputDescricao" name="<?php echo $desc_column; ?>" rows="3" placeholder="Detalhes sobre o uso ou tipo de equipamento."></textarea>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const resetModal = () => {
        modalTitle.textContent = 'Cadastrar Novo Recurso';
        modalAction.value = 'insert';
        modalId.value = '';
        inputNome.value = ''; // Limpa o campo
        inputDescricao.value = ''; // Limpa a descrição
    ...
    modalElement.addEventListener('show.bs.modal', function (event) {
        ...
        if (button && button.classList.contains('btn-edit')) {
            ...
            modalTitle.textContent = 'Editar Recurso (ID: ' + id + ')';
