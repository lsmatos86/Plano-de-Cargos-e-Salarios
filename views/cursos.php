...
$table_name = 'cursos';
$id_column = 'cursoId';
$name_column = 'cursoNome';
$desc_column = 'cursoDescricao';
    ...
    if (!empty($nome)) {
        try {
            if ($action === 'insert') {
                ...
                $message = "Curso ID {$id} atualizado com sucesso!";
                $message_type = 'success';
            }
        } catch (Exception $e) {
        ...
        $message = "O nome do Curso não pode ser vazio.";
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    ...
    if (deleteRecord($pdo, $table_name, $id_column, $id)) {
        ...
        $message = "Erro ao excluir: O curso ID {$id} está vinculado a um Cargo.";
        $message_type = 'danger';
    }
    ...
    <div class="row mb-3">
        <div class="col-md-4">
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#cadastroModal" id="btnNovoCadastro">
                <i class="fas fa-plus"></i> Inserir Novo Curso
            </button>
        </div>
        <div class="col-md-8">
            <form method="GET" class="d-flex">
                ...
                <?php if (!empty($params['term'])): ?>
                    <a href="cursos.php" class="btn btn-outline-danger ms-2" title="Limpar Filtro"><i class="fas fa-times"></i></a>
                <?php endif; ?>
            </form>
        </div>
    </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="cursos.php?action=delete&id=<?php echo $row[$id_column]; ?>" 
                                       ...
                                       title="Excluir"
                                       onclick="return confirm('ATENÇÃO: Este curso pode estar vinculado a um Cargo. Deseja realmente excluir?');">
        ...
        <div class="modal-content">
            <form method="POST" id="formCadastro">
                ...
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="cadastroModalLabel">Cadastrar Novo Curso</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="inputNome" class="form-label">Nome do Curso</label>
                        <input type="text" class="form-control" id="inputNome" name="<?php echo $name_column; ?>" placeholder="Ex: Fruticultura, Liderança" required>
                    </div>
                    <div class="mb-3">
                        <label for="inputDescricao" class="form-label">Descrição (Opcional)</label>
                        <textarea class="form-control" id="inputDescricao" name="<?php echo $desc_column; ?>" rows="3" placeholder="Detalhes sobre o conteúdo ou o instrutor."></textarea>
                    </div>
                </div>
